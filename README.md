...
**Motion & graphics:** count-up numbers, sliding gauge markers, water cups that fill in a cascade, growing timeline journey with popping milestones, animated macro/ring charts, staggered card entrances, drifting ambient glow orbs, nav-tab icons and meal emojis — all pure CSS/JS, and fully disabled for `prefers-reduced-motion` users (tests run with `?nofx=1`).
